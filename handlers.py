        ...
        if not games.data[message.from_id].start:
            ...
            if games.data[message.from_id].count: # если сыграли хотябы одну игру
                if games.data[message.from_id].win and games.data[message.from_id].oppobag > 49: # если в прошлой игре победил бот и у человека в мешке больше 49 конфет
                    ...
                    await message.answer(f'Жаждешь реванша? Я не против.\nУ меня в мешке {change_word(games.data[message.from_id].botbag + games.data[message.from_id].total)}, у тебя в мешке {change_word(games.data[message.from_id].oppobag)}. Этого не достаточно.\nТак уж и быть, положу в вазу {change_word(games.data[message.from_id].total)}.\nБрать можно не больше {games.data[message.from_id].lot} конфет.\nСколько конфет забераешь?')
                elif games.data[message.from_id].botbag < 50:  # если у бота в мешке меньше 50 конфет
                    await message.answer(f'У меня в мешке {change_word(games.data[message.from_id].botbag)}. Этого недостаточно, чтобы продолжить игру.\nУ тебя в мешке {change_word(games.data[message.from_id].oppobag)}.\n Если хочешь продолжить игру, можешь добавить мне {change_word(50 - games.data[message.from_id].botbag)}, и продолжим игру. Если согласен введи {50 - games.data[message.from_id].botbag}.')
                else:
                    games.data[message.from_id].botContribution()
                    await message.answer(f'В этот раз я своего не упущу!!\nУ тебя в мешке {change_word(games.data[message.from_id].oppobag)}, у меня в мешке {change_word(games.data[message.from_id].botbag + games.data[message.from_id].total)}. Я кладу в вазу {change_word(games.data[message.from_id].total)}.\nБрать можно не больше {games.data[message.from_id].lot} конфет.\nСколько конфет забераешь?')
            else:
...
async def mes_all(message: Message):
    ...
    elif not games.data[message.from_id].start and num.isdigit() and int(num) == 50 - games.data[message.from_id].botbag: # если игра не начата и введёное число равно недастающему боту количеству конфет
